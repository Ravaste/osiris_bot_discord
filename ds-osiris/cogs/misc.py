...
class Misc(commands.Cog):
    def __init__(self, bot):
        ...
        self.cd_image = datetime.now()
        self.image_genere = 0
        self.premiere_img = datetime.now()
    ...
    async def changement_status(self):
        statuses = [
            ...
            "Écrivant des devinettes",
            "Explorant de nouvelles îles",
            "Naviguant sur les mers",
            "En quête d'aventures",
            "Réfléchissant à la vie de pirate",
            "Partageant des histoires de pirates",
            "avec les reufs",
            "Buveur de rhum",
            "Chantant des shanties",
            "Sous le soleil",
            "À la taverne",
            "Sur le pont",
            ...
            "Sur mon navire",
            "Sur le PC de Berkant",
            "Dansant sur le pont",
            "Regardant les étoiles",
            "A League of Legends avec Gankplank Top",
            "Préparant une embuscade",
            "Vole un Athéna",
            "Cherche la beauté des étoiles",
            "Cherche sa moitié (Une femme de préférence)",
            "Écoute le chant des sirènes",
            "Est dans la merde",
            "Fais des blagues nulles",
            "Écoute Alan Parson",
            "Cache le trésor de Berkant",
            "Fais des rimes",
            "Cache son rhum",
    ...
    async def on_message(self, message):
        ...
        if self.bot.defis_evoque is not None:
    ...
    async def _handle_osiris_reply(self, message):
        ...
        # Override channel devinette (si aucun user override ne l'a déjà fait)
        if message.channel.id == 1370337501738434650:
            facon_etre = (
    ...
    async def nuit(self, ctx):
        ...
        """
        heure_actu = datetime.now().strftime('%H')
        if int(heure_actu) > 8 and int(heure_actu) < 22:
            await ctx.send(f"Tu nous fais quoi le reuf, il est {heure_actu}H pas le moment de dormir")
        elif int(heure_actu) < 8 and int(heure_actu) > 22:
            await ctx.send(f"Je te souhaite une bonne nuit")
    ...
    @app_commands.command(
        name="nsfw",
        description="Recherche une image NSFW sur rule34.xxx",
    )
    @app_commands.guilds(discord.Object(id=1064626848903999571))
    @app_commands.describe(tags="Les tags pour la recherche")
    async def nsfw(self, interaction: discord.Interaction, tags: str):
        tags_interdit = {
            "loli", "shota", "rape", "incest", "bestiality", "zoophilia", "child", "underage",
            "toddler", "cub", "guro", "scat", "necrophilia", "abuse", "snuff", "vore",
            "torture", "execution", "lolicon", "shotacon"
        }

        tags_lower = tags.lower()

        if any(tag in tags_lower for tag in tags_interdit):
            await interaction.response.send_message(
                "⛔ *Je suis déçu de toi* ! Ces tags sont interdits.", ephemeral=True
            )
            return

        if interaction.guild and not interaction.channel.is_nsfw():
            await interaction.response.send_message(
                "🔞 Cette commande est réservée aux salons NSFW.", ephemeral=True
            )
            return
        ...
        safe_tags = urllib.parse.quote(formatted_tags)
        url = (
            f"https://api.rule34.xxx/index.php?page=dapi&s=post&q=index"
            f"&json=1&tags={safe_tags}&limit=100"
        )
            ...
            try:
                async with session.get(url, headers=headers) as response:
                    if response.status != 200:
                        await interaction.followup.send(f"⚠️ Erreur API: Code {response.status}")
                        return
                    try:
                ...
                await interaction.followup.send(f"⚠️ Erreur de connexion : {e}")
                return

        if not data or not isinstance(data, list):
            await interaction.followup.send(f"Aucune image trouvée pour : `{tags}` 🤷‍♂️")
            return
        ...
        if isinstance(post, dict):
            ...
            if img_url:
                ...
                await interaction.followup.send("Erreur : L'API a renvoyé un résultat sans image.")
        else:
            await interaction.followup.send("Erreur de format inattendu de la part de Rule34.")
