...
def get_current_prog():
    try:
        ...
        print(f"Erreur get_current_prog : {e}")
        return 1, PROG_DATA[0]
...
class Sport(commands.Cog):
    ...
    async def rappel_sport_task(self):
        try:
            ...
            print(f"Erreur rappel_sport_task : {e}")

    @rappel_sport_task.error
    async def rappel_sport_error(self, error):
        print(f"Erreur tâche sport : {error}")
        # La tâche repart automatiquement à la prochaine itération
    ...
    async def on_message(self, message):
        ...
        try:
            if (
                ...
                if self.rappel_sport_task.is_running():
                    self.rappel_sport_task.stop()
                await message.channel.send(
                    "💪 **Bien joué !** Repose-toi bien, à demain pour la suite du programme !"
                )
        except Exception as e:
            print(f"Erreur on_message sport : {e}")
